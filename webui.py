...
from frontend.task_detail_page import render_task_detail_page
...
def _build_backend_client() -> BackendApiClient:
    ...
    token = st.session_state.get("board_api_token", os.getenv("BOARD_ASSISTANT_API_TOKEN", "dev-token"))
    return BackendApiClient(base_url=base_url, token=token)


def _render_eval_page(client: BackendApiClient) -> None:
    """渲染评估结果页面。"""
    st.subheader("评估结果")
    project_id = st.text_input("项目 ID", value=st.session_state.get("eval_project_id", ""))
    board_platform = st.selectbox("开发板平台", ["huawei", "phytium", "other"], index=2)
    scenario_name = st.text_input("评估场景", value="sensor_monitor_demo")
    if st.button("启动评估任务", key="run_eval"):
        try:
            ...
            st.session_state["eval_run_result"] = result
            st.success("评估任务已创建")
            st.json(result)
        except Exception as exc:
            st.error(f"评估任务创建失败: {exc}")

    eval_task_id = st.text_input(
        "评估任务 ID",
        value=(st.session_state.get("eval_run_result") or {}).get("evaluationTaskId", ""),
    )
    if st.button("生成评估报告", key="gen_eval_report") and eval_task_id.strip():
        try:
            report = client.generate_eval_report(
                {"evaluationTaskId": eval_task_id.strip(), "includeImprovementProposal": True}
            )
            st.session_state["eval_report"] = report
            st.success("评估报告已生成")
            st.json(report)
        except Exception as exc:
            st.error(f"评估报告生成失败: {exc}")
...
def main(is_admin: bool, usname: str) -> None:
    ...
    if app_page == "板卡助手":
        render_board_assistant_page(client)
        return
    if app_page == "评估结果":
        _render_eval_page(client)
        return
    if app_page == "任务详情":
        task_id = st.query_params.get("taskId", "")
        render_task_detail_page(client, task_id)
        return
