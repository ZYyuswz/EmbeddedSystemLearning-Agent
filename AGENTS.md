...
- 不要改动未指定的业务代码
- 只在任务范围内读取和编辑代码；本仓库默认允许改动 `backend/`、`frontend/`、`src/`、`web/`、`api/`、`prompts/` 及需求明确指向的相关目录
- 若仓库实际目录与上述不一致，以“需求明确指定的模块/文件”为准，禁止越界修改
...
- 可复用的代码抽象成方法
- 类内可复用的方法抽象成类内的工具方法
- 跨类或跨模块可复用的方法抽象成 `utils/`、`shared/` 或项目既有公共层
- 每个新增或修改的方法级函数、类、对象都要补充注释，注释风格遵循对应语言与框架规范（如 Java 使用 Javadoc、Python 使用 docstring、TS/JS 使用 TSDoc/JSDoc）
- 新增或修改函数、类等对象时必须同步更新注释
- 在记录日志时，除了专有名词可以用英文缩写，其他内容全部用中文

## Post Coding

- 每次如果有新的后端接口添加或接口发生改动，都需要对应更新 `prompts/sirius-api.postman.json`
- 如果是前端接口消费方式变更（路径、参数、响应字段映射、鉴权头等），也需要同步检查并更新上述文件或相关 API 文档
- 保持该文件原本的规则和格式
...
- Agent 1：需求与接口设计 Agent  
  负责需求澄清、边界定义、接口设计、数据模型草案、验收标准（AC），覆盖前端与后端影响面。
- Agent 2：代码改动规划 Agent  
  负责输出文件/类/方法级改动计划、伪代码、数据库与前端状态/路由/交互变更、实施顺序、回滚点。
- Agent 3：开发实现 Agent  
  负责严格按 Agent 2 的计划实现代码与测试，并记录与计划差异。
- Agent 4：测试与质量 Agent  
  负责基于 AC 编写测试并自动执行测试与回归，输出缺陷分级与放行结论，覆盖前端与后端。
...
- Agent 4 只允许改动测试相关文件（如 `src/test/`、`tests/`、`frontend`/`backend` 下测试目录及测试配置），不得改动业务实现代码。
- Agent 4 编写的测试必须符合最佳实践（可读、可复现、稳定、覆盖正常/异常/边界路径）。
- Agent 4 必须自动执行至少一条与改动栈匹配的测试命令并记录结果（如 `mvn test`、`pytest`、`npm test`、`pnpm test` 等）。
- Agent 4 目标测试覆盖率不低于 80%（若受历史基线或工具限制无法达成，需在结论中说明差距与补齐计划）。
...
- 只有 Agent 3 可以修改业务代码。
- Agent 4 在提交测试结论前必须完成“编写测试 + 自动执行”。
